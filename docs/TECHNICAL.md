...
3. 如果是 `去完成`，点击按钮。只要重新 OCR 后任务状态仍是 `去完成`，就继续点击，不按点击次数或广告轮数触发整轮重启。
4. 点击后通过右上角关闭按钮判断是否进入广告页；如果没有检测到关闭按钮，等待后重新 OCR 当前任务状态。若仍能重新定位或行兜底推断当前任务的 `去完成`，继续点击；若无法重新定位，才视为流程异常。
...
11. OCR 查找奖励确认弹窗：
    ...
    - 如果 10 秒内未命中奖励确认弹窗，会记录最近一次 OCR 预览并继续复查任务状态。
...
- 如果当前任务状态仍是 `去完成`，即使已经点击多次或执行多轮广告，也继续点击，不触发整轮重启。
...
- 点击任务右侧 `去完成`：
  ...
  - 失败处理：等待 1 秒后重新 OCR 当前屏任务状态，若仍能重新定位 `去完成`，继续点击，不按点击次数触发重启。
...
- 关闭广告打开浏览弹窗：
  ...
  - 最大次数：3 次。
- 关闭广告后的奖励确认弹窗：
  - 优先条件：识别到 `知道了` 或 `知道`，直接点击 OCR 文本中心。
