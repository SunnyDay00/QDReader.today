...
3. 如果是 `去完成`，点击按钮。只要重新 OCR 后任务状态仍是 `去完成`，就在安全窗口内继续点击。
4. 点击后通过右上角关闭按钮判断是否进入广告页；如果没有检测到关闭按钮，等待后重新 OCR 当前任务状态。若仍能重新定位或行兜底推断当前任务的 `去完成`，继续点击；但单轮 `去完成` 点击最多 6 次或 60 秒，超过后视为步骤卡住并触发整轮重启。
5. 确认进入广告页后，用 OpenCV 找右上角关闭按钮并点击。
6. 等待弹窗，OCR 查找浏览动作文案并点击；如果没有识别到弹窗，等待后重新检测右上角关闭按钮并再次点击，最多 3 次。
7. 等 18 秒。
8. OCR 查找 `恭喜已获得奖励`、`恭喜获得奖励`、`恭喜获得`、`已获得奖励` 或 `奖励已到账`。
9. 系统返回到广告页。
10. 再次用 OpenCV 找右上角关闭按钮并点击。
11. OCR 查找奖励确认弹窗：
    - 优先识别并点击 `知道了`、`我知道了`、`知道啦` 或 `知道`。
    - 如果只识别到 `恭喜获得`、`获得奖励` 或中心区域的 `订阅券`、`章节卡`、`点币` 等奖励文案，但未定位到 `知道了`，则按弹窗下方中心确认按钮坐标兜底点击。
    - 如果 10 秒内未命中奖励确认弹窗，会先复查任务列表是否可操作；如果任务列表不可操作，按弹窗确认按钮常见位置兜底点击一次并再次验证任务列表，仍失败则触发整轮重启。
12. 回到福利中心当前屏，复查任务状态。若仍是 `去完成`，继续下一轮；直到 `已领取` 或状态无法确认。

三项广告奖励任务按固定顺序执行：

- 某项任务启动时如果已经是 `已领取`，该任务直接视为成功并跳过。
- 某个步骤重试耗尽、OCR 连续失败、广告页关闭按钮失败、浏览弹窗失败、等待奖励失败，或无法重新确认当前任务状态时，该轮 flow 会返回 `restartRequested = true`。
- 如果当前任务状态仍是 `去完成`，不会因为固定 3 次点击立即重启；但同一轮点击在 6 次或 60 秒内始终不能进入广告页时，会触发整轮重启。
...
- 关闭广告后的奖励确认弹窗：
  ...
  - 遮挡恢复：如果未识别到奖励弹窗，但当前任务列表也无法重新定位，会按奖励弹窗确认按钮常见位置兜底点击一次，再次验证任务列表。
